...
$output .= "\t\t\t\t" . '<div><label for="wpsl-search-input">' . esc_html( $wpsl->i18n->get_translation( 'search_label', __( 'Your location', 'wp-store-locator-extenders' ) ) ) . '</label></div>' . "\r\n";
...
if ( $wpsl_settings['radius_dropdown'] || $wpsl_settings['results_dropdown']  ) {
    ...
    if ( $wpsl_settings['radius_dropdown'] ) {
        ...
        $output .= "\t\t\t\t\t" . '<label for="wpsl-radius-dropdown">' . esc_html( $wpsl->i18n->get_translation( 'radius_label', __( 'Search radius', 'wp-store-locator-extenders' ) ) ) . '</label>' . "\r\n";
    ...
    if ( $wpsl_settings['results_dropdown'] ) {
        ...
        $output .= "\t\t\t\t\t" . '<label for="wpsl-results-dropdown">' . esc_html( $wpsl->i18n->get_translation( 'results_label', __( 'Results', 'wp-store-locator-extenders' ) ) ) . '</label>' . "\r\n";
...
$output .= "\t\t" . '<div id="wpsl-extend-search-wrap">' . "\r\n";
$output .= apply_filters( 'wpsl_extend_search_input', '' ) . "\r\n";
$output .= "\t\t" . '</div>' . "\r\n";

$output .= "\t\t\t\t" . '<div class="wpsl-search-btn-wrap"><input id="wpsl-search-btn" type="submit" value="' . esc_attr( $wpsl->i18n->get_translation( 'search_btn_label', __( 'Search', 'wp-store-locator-extenders' ) ) ) . '"></div>' . "\r\n";
...
if ( $wpsl_settings['show_credits'] ) { 
    $output .= "\t" . '<div class="wpsl-provided-by">'. sprintf( __( "Search provided by %sWP Store Locator%s", "wp-store-locator-extenders" ), "<a target='_blank' href='https://wpstorelocator.co'>", "</a>" ) .'</div>' . "\r\n";
}
